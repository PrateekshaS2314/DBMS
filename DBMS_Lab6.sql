...
-- 'root@localhost', 'LAPTOP-P28LNMN8', '8.4.6', '2025-09-29 11:32:44'
...
-- Write the SQL Statement for the above Question
select job_id, avg(Salary) from employees as AvgSalary group by job_id;
...
-- Write the SQL Statement for the above Question
select department_id, sum(salary) as total from employees group by department_id having sum(salary)>50000;
...
-- Write the SQL Statement for the above Question
SELECT department_id, MAX(salary) AS max_salary, MIN(salary) AS min_salary, AVG(salary) AS avg_salary FROM employees 
...
-- Write the SQL Statement for the above Question
SELECT EXTRACT(YEAR FROM hire_date) AS hire_year, COUNT(*) AS employee_count FROM employees GROUP BY EXTRACT(YEAR FROM hire_date);
...
-- Write the SQL Statement for the above Question
SELECT e.job_id, j.job_title, AVG(e.salary) AS avg_salary FROM employees e JOIN jobs j ON e.job_id = j.job_id GROUP BY e.job_id, j.job_title
...
-- Write the SQL Statement for the above Question
SELECT department_id, AVG(commission_pct) AS avg_commission_pct FROM employees WHERE commission_pct IS NOT NULL
...
-- Write the SQL Statement for the above Question
SELECT manager_id, COUNT(*) AS employee_count FROM employees WHERE manager_id IS NOT NULL GROUP BY manager_id ORDER BY employee_count DESC;
